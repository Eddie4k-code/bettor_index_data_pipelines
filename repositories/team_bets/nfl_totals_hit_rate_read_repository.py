"""Point-in-time reads from ``nfl_team_bet_totals_hit_rates``."""
...
class NflTotalsHitRateReadRepository(NflTotalsHitRateReadRepositoryInterface):
    ...
    def fetch_pregame_hit_rates(
        ...
        *,
        ...
        rows = (
            self.db.query(NflTeamBetTotalsHitRates)
            .filter(
                NflTeamBetTotalsHitRates.market_key == "totals",
                NflTeamBetTotalsHitRates.bookmaker.in_(ALLOWED_BOOKMAKERS),
            )
            .all()
        )
        return [
