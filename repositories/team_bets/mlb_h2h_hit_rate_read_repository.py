"""Point-in-time reads from ``mlb_team_bet_hit_rates``."""
...
class MlbH2hHitRateReadRepository(MlbH2hHitRateReadRepositoryInterface):
    ...
    def fetch_pregame_hit_rates(
        ...
        *,
        ...
        rows = (
            self.db.query(MlbTeamBetHitRates)
            .filter(
                MlbTeamBetHitRates.market_key == "h2h",
                MlbTeamBetHitRates.bookmaker.in_(ALLOWED_BOOKMAKERS),
            )
            .all()
        )
        return [
