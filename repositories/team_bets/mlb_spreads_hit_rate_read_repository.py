"""Point-in-time reads from ``mlb_team_bet_spreads_hit_rates``."""
...
class MlbSpreadsHitRateReadRepository(MlbSpreadsHitRateReadRepositoryInterface):
    ...
    def fetch_pregame_hit_rates(
        ...
        *,
        ...
        rows = (
            self.db.query(MlbTeamBetSpreadsHitRates)
            .filter(
                MlbTeamBetSpreadsHitRates.market_key == "spreads",
                MlbTeamBetSpreadsHitRates.bookmaker.in_(ALLOWED_BOOKMAKERS),
            )
            .all()
        )
        return [
