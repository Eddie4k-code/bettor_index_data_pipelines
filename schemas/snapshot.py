"""Shared snapshot request/result shapes and base pregame record fields."""
...
class SnapshotInsertResult(BaseModel):
    """Append-only persistence outcome for one batch of snapshot rows."""
...
class MarketSnapshotBuildResult(BaseModel):
    """Builder output before persistence."""

    model_config = ConfigDict(frozen=True)

    records: list["PregameSnapshotRecordBase"]
    skipped_leakage: int
    skipped_missing_hit_rate: int


class PregameSnapshotRecordBase(BaseModel):
    """Fields shared by every persisted pregame snapshot row."""
