"""NFL H2H post-game grade orchestrator."""
...
from interfaces.nfl_h2h_grade_pipeline_interface import NflH2hGradePipelineInterface
...
class NflH2hGradePipeline(NflH2hGradePipelineInterface):
    def __init__(
        self,
        *,
        snapshot_read_repo: NflH2hSnapshotReadRepositoryInterface,
        games_repo: GamesReadRepositoryInterface,
        grade_repo: NflH2hGradeRepositoryInterface,
    ...
    def run(self, request: GradeRequest) -> GradeRunResult:
        if request.sport_key != SPORT_KEY or request.market_key != MARKET_KEY:
            ...
            raise ValueError(
                f"NflH2hGradePipeline requires sport_key={SPORT_KEY!r} and "
                f"market_key={MARKET_KEY!r}, got {request.sport_key!r}/{request.market_key!r}",
            )

        now = self.now_fn()
        logger.info(
            "Starting NFL H2H grade run event_id=%s",
            request.event_id,
        )
        ...
        logger.info(
            "NFL H2H grade run complete event_id=%s candidates=%s graded=%s "
