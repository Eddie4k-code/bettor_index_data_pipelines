"""Nba Totals pregame snapshot orchestrator."""
...
from interfaces.nba_totals_snapshot_pipeline_interface import NbaTotalsSnapshotPipelineInterface
...
class NbaTotalsSnapshotPipeline(NbaTotalsSnapshotPipelineInterface):
    def __init__(
        self,
        *,
        odds_repo: FeaturedOddsReadRepositoryInterface,
        hit_rate_repo: NbaTotalsHitRateReadRepositoryInterface,
        snapshot_repo: NbaTotalsSnapshotRepositoryInterface,
    ...
    def run(self, request: SnapshotRequest) -> SnapshotRunResult:
        if request.sport_key != SPORT_KEY or request.market_key != MARKET_KEY:
            ...
            raise ValueError(
                f"NbaTotalsSnapshotPipeline requires sport_key={SPORT_KEY!r} and "
                f"market_key={MARKET_KEY!r}, got {request.sport_key!r}/{request.market_key!r}",
            )

        observation_time = request.observation_time
        logger.info(
            "Starting NBA totals snapshot run observation_time=%s",
            observation_time,
        )
        ...
        logger.info(
            "NBA totals snapshot run complete observation_time=%s candidates=%s snapshotted=%s "
