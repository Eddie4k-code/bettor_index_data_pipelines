"""NFL spreads post-game grade orchestrator."""
...
from interfaces.nfl_spreads_grade_pipeline_interface import NflSpreadsGradePipelineInterface
...
class NflSpreadsGradePipeline(NflSpreadsGradePipelineInterface):
    def __init__(
        self,
        *,
        snapshot_read_repo: NflSpreadsSnapshotReadRepositoryInterface,
        games_repo: GamesReadRepositoryInterface,
        grade_repo: NflSpreadsGradeRepositoryInterface,
    ...
    def run(self, request: GradeRequest) -> GradeRunResult:
        if request.sport_key != SPORT_KEY or request.market_key != MARKET_KEY:
            ...
            raise ValueError(
                f"NflSpreadsGradePipeline requires sport_key={SPORT_KEY!r} and "
                f"market_key={MARKET_KEY!r}, got {request.sport_key!r}/{request.market_key!r}",
            )

        now = self.now_fn()
        logger.info(
            "Starting NFL spreads grade run event_id=%s",
            request.event_id,
        )
        ...
        for snapshot in candidates:
            ...
            record = self.builder.build(
                snapshot=snapshot,
                game=game,
                record_cls=NflSpreadsGradeRecord,
                graded_at=graded_at,
        ...
        logger.info(
            "NFL spreads grade run complete event_id=%s candidates=%s graded=%s "
