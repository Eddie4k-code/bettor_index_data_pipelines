"""NBA spreads post-game grade orchestrator."""
...
from interfaces.nba_spreads_grade_pipeline_interface import NbaSpreadsGradePipelineInterface
...
class NbaSpreadsGradePipeline(NbaSpreadsGradePipelineInterface):
    def __init__(
        self,
        *,
        snapshot_read_repo: NbaSpreadsSnapshotReadRepositoryInterface,
        games_repo: GamesReadRepositoryInterface,
        grade_repo: NbaSpreadsGradeRepositoryInterface,
    ...
    def run(self, request: GradeRequest) -> GradeRunResult:
        if request.sport_key != SPORT_KEY or request.market_key != MARKET_KEY:
            ...
            raise ValueError(
                f"NbaSpreadsGradePipeline requires sport_key={SPORT_KEY!r} and "
                f"market_key={MARKET_KEY!r}, got {request.sport_key!r}/{request.market_key!r}",
            )

        now = self.now_fn()
        logger.info(
            "Starting NBA spreads grade run event_id=%s",
            request.event_id,
        )
        ...
        logger.info(
            "NBA spreads grade run complete event_id=%s candidates=%s graded=%s "
