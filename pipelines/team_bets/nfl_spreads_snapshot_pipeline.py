"""Nfl Spreads pregame snapshot orchestrator."""
...
from interfaces.nfl_spreads_snapshot_pipeline_interface import NflSpreadsSnapshotPipelineInterface
...
class NflSpreadsSnapshotPipeline(NflSpreadsSnapshotPipelineInterface):
    def __init__(
        self,
        *,
        odds_repo: FeaturedOddsReadRepositoryInterface,
        hit_rate_repo: NflSpreadsHitRateReadRepositoryInterface,
        snapshot_repo: NflSpreadsSnapshotRepositoryInterface,
    ...
    def run(self, request: SnapshotRequest) -> SnapshotRunResult:
        if request.sport_key != SPORT_KEY or request.market_key != MARKET_KEY:
            ...
            raise ValueError(
                f"NflSpreadsSnapshotPipeline requires sport_key={SPORT_KEY!r} and "
                f"market_key={MARKET_KEY!r}, got {request.sport_key!r}/{request.market_key!r}",
            )

        observation_time = request.observation_time
        logger.info(
            "Starting NFL spreads snapshot run observation_time=%s",
            observation_time,
        )
        ...
        logger.info(
            "NFL spreads snapshot run complete observation_time=%s candidates=%s snapshotted=%s "
