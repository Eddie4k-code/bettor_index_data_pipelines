"""NBA totals post-game grade orchestrator."""
...
from interfaces.nba_totals_grade_pipeline_interface import NbaTotalsGradePipelineInterface
...
class NbaTotalsGradePipeline(NbaTotalsGradePipelineInterface):
    def __init__(
        self,
        *,
        snapshot_read_repo: NbaTotalsSnapshotReadRepositoryInterface,
        games_repo: GamesReadRepositoryInterface,
        grade_repo: NbaTotalsGradeRepositoryInterface,
    ...
    def run(self, request: GradeRequest) -> GradeRunResult:
        if request.sport_key != SPORT_KEY or request.market_key != MARKET_KEY:
            ...
            raise ValueError(
                f"NbaTotalsGradePipeline requires sport_key={SPORT_KEY!r} and "
                f"market_key={MARKET_KEY!r}, got {request.sport_key!r}/{request.market_key!r}",
            )

        now = self.now_fn()
        logger.info(
            "Starting NBA totals grade run event_id=%s",
            request.event_id,
        )
        ...
        logger.info(
            "NBA totals grade run complete event_id=%s candidates=%s graded=%s "
