"""NBA H2H post-game grade orchestrator."""
...
from interfaces.nba_h2h_grade_pipeline_interface import NbaH2hGradePipelineInterface
...
class NbaH2hGradePipeline(NbaH2hGradePipelineInterface):
    def __init__(
        self,
        *,
        snapshot_read_repo: NbaH2hSnapshotReadRepositoryInterface,
        games_repo: GamesReadRepositoryInterface,
        grade_repo: NbaH2hGradeRepositoryInterface,
    ...
    def run(self, request: GradeRequest) -> GradeRunResult:
        if request.sport_key != SPORT_KEY or request.market_key != MARKET_KEY:
            ...
            raise ValueError(
                f"NbaH2hGradePipeline requires sport_key={SPORT_KEY!r} and "
                f"market_key={MARKET_KEY!r}, got {request.sport_key!r}/{request.market_key!r}",
            )

        now = self.now_fn()
        logger.info(
            "Starting NBA H2H grade run event_id=%s",
            request.event_id,
        )
        ...
        logger.info(
            "NBA H2H grade run complete event_id=%s candidates=%s graded=%s "
