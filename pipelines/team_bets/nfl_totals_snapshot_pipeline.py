"""Nfl Totals pregame snapshot orchestrator."""
...
from interfaces.nfl_totals_hit_rate_read_repository_interface import NflTotalsHitRateReadRepositoryInterface
from interfaces.nfl_totals_snapshot_pipeline_interface import NflTotalsSnapshotPipelineInterface
...
class NflTotalsSnapshotPipeline(NflTotalsSnapshotPipelineInterface):
    def __init__(
        self,
        *,
        odds_repo: FeaturedOddsReadRepositoryInterface,
        hit_rate_repo: NflTotalsHitRateReadRepositoryInterface,
        snapshot_repo: NflTotalsSnapshotRepositoryInterface,
    ...
    def run(self, request: SnapshotRequest) -> SnapshotRunResult:
        if request.sport_key != SPORT_KEY or request.market_key != MARKET_KEY:
            ...
            raise ValueError(
                f"NflTotalsSnapshotPipeline requires sport_key={SPORT_KEY!r} and "
                f"market_key={MARKET_KEY!r}, got {request.sport_key!r}/{request.market_key!r}",
            )

        observation_time = request.observation_time
        logger.info(
            "Starting NFL totals snapshot run observation_time=%s",
            observation_time,
        )
        ...
        logger.info(
            "NFL totals snapshot run complete observation_time=%s candidates=%s snapshotted=%s "
...
def _index_hit_rates(
    hit_rate_rows: list[TeamBetTotalsHitRateRow],
) -> dict[tuple[str, str, str, str], TeamBetTotalsHitRateRow]:
    return {
